...
$total_query = "SELECT COUNT(*) as total FROM gas_requests WHERE user_id = ?";
...
$query = "SELECT gr.*, o.outlet_name, ct.name as cylinder_type, ct.weight
          FROM gas_requests gr
          JOIN outlets o ON gr.outlet_id = o.outlet_id
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">My Requests</li>
                    </ol>
        ...
        <div class="main-content">
            <div class="requests-container">
                <div class="requests-header">
                    <h5 class="d-flex justify-content-center">My Gas Requests</h5>
                    <a href="gas_request.php" class="btn btn-primary">
                ...
                <?php if ($requests->num_rows > 0): ?>
                    <div class="requests-table-wrapper">
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Token
                                        Number</th>
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Outlet
                                    </th>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Request
                                        Date</th>
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Pickup
                                        Date</th>
                            ...
                            <tbody>
                                <?php while ($request = $requests->fetch_assoc()): ?>
                                    <tr class="request-row status-<?php echo strtolower($request['status']); ?>">
                                        <td><?php echo htmlspecialchars($request['token_number']); ?></td>
                                        <td><?php echo htmlspecialchars($request['outlet_name']); ?></td>
                                        <td>
                                            <?php echo htmlspecialchars($request['cylinder_type']); ?>
                                            (<?php echo htmlspecialchars($request['weight']); ?>kg)
                                        </td>
                                        <td><?php echo htmlspecialchars($request['quantity']); ?></td>
                                        <td><?php echo date('Y-m-d H:i', strtotime($request['request_date'])); ?></td>
                                        <td>
                                            <?php if ($request['scheduled_pickup_date']): ?>
                                                <?php echo date('Y-m-d H:i', strtotime($request['scheduled_pickup_date'])); ?>
                                            <?php else: ?>
                                        ...
                                        <td class="actions-cell">
                                            <button class="btn btn-icon"
                                                onclick="viewDetails(<?php echo $request['request_id']; ?>)">
                ...
                <?php else: ?>
                    <div class="no-requests">
                        <i class="fas fa-file-alt no-requests-icon"></i>
                        <p>You haven't made any gas requests yet.</p>
                        <a href="gas_request.php" class="btn btn-primary">Make Your First Request</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            function viewDetails(requestId) {
                ...
                fetch(`get_request_details.php?id=${requestId}`)
                    ...
                    <div class="request-info">
                        ...
                        <p><strong>Cylinder Type:</strong> ${data.cylinder_type}</p>
                        <p><strong>Quantity:</strong> ${data.quantity}</p>
                        <p><strong>Request Date:</strong> ${data.request_date}</p>
                        <p><strong>Scheduled Pickup:</strong> ${data.scheduled_pickup_date || 'Not scheduled'}</p>
                        <p><strong>Status:</strong> ${data.status}</p>
                        ${data.payment_status ? `<p><strong>Payment Status:</strong> ${data.payment_status}</p>` : ''}
                        ${data.empties_received ? '<p><strong>Empties:</strong> Received</p>' : ''}
                    </div>
                `;
